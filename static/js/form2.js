...
var vents={
    ...
    calculate_invoice: function () {
        ...
        $.each(this.items.productos, function (pos, dict) {
            dict.pos = pos; 
            if (dict.impuesto == true) {
                dict.subtotal = dict.cant *(0.19 * parseFloat(dict.precio_venta) + parseFloat(dict.precio_venta))
        ...
        console.log(vents.items.productos);
        ...
        tblProducts = $('#tblProducts').DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function (data, type, row) {
                        return '<input type="checkbox" name="impuesto" id="impuesto" class="form-check-input" value="' + row.impuesto + '">';
                    }
                },
                {
                    targets: [-1],
...
$(function () {
    ...
    $('#tblProducts tbody')
        ...
        vents.calculate_invoice();
        $('td:eq(6)', tblProducts.row(tr.row).node()).html('$' + vents.items.productos[tr.row].subtotal.toFixed(2));
        })
        .on('change', 'input[name="impuesto"]', function () {
            //console.clear();
            if(this.checked){
                var impuesto = true;
                var tr = tblProducts.cell($(this).closest('td, li')).index();
                vents.items.productos[tr.row].impuesto = impuesto;
                vents.calculate_invoice();
                $('td:eq(6)', tblProducts.row(tr.row).node()).html('$' + vents.items.productos[tr.row].subtotal.toFixed(2));
            }
            else {                
    ...
    $('form').on('submit', function (e) {
        ...
        vents.items.fecha_compra = $('input[name="fecha"]').val();
